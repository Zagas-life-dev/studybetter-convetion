...
export async function GET(request: NextRequest) {
  try {
    ...
    const actionType = searchParams.get("action_type") || "summary"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!action_type || !["summary", "download"].includes(action_type)) {
      return new NextResponse(JSON.stringify({ error: "Invalid action_type" }), { status: 400, headers: { 'Content-Type': 'application/json' } })
    }
